...
TinyDeepLearning是一个使用Python从底层实现的简易深度学习框架。API风格模仿Keras，底层使用Tensor自动进行梯度反向传播，计算模型实现了序列模型和计算图模型，其中计算图模型则可以实现任意复杂的图计算(包括残差连接、共享参数等)。 该框架主要用于探究深度学习框架的内部实现原理。
